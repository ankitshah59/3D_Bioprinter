...
namespace CL3_IF_DllSample
{
    public partial class MainForm : Form
    {
        ...
        private const decimal PositionTolerance = 0.001m; // adjust to your units

        private static bool IsAtTarget(decimal current, decimal target, decimal tol)
            => Math.Abs(current - target) <= tol;

        private static async System.Threading.Tasks.Task WaitWhileMovingAsync(Func<bool> isMoving)
        {
            while (isMoving())
        ...
        */
        ...
        public async void Move_Down(String name, Thorlabs.MotionControl.IntegratedStepperMotorsCLI.LongTravelStage controller)
        {
            ...
            await WaitWhileMovingAsync(() => controller.Status.IsMoving);

            decimal target = positionInitial + Convert.ToDecimal(textBox16.Text);
            if (IsAtTarget(controller.Position, target, PositionTolerance))
                port.Write("B");

        }
        public async void Move_Up(String name, Thorlabs.MotionControl.IntegratedStepperMotorsCLI.LongTravelStage controller)
        {
        ...
        public async void Move_Clock(String name, Thorlabs.MotionControl.KCube.DCServoCLI.KCubeDCServo controllerY)
        {
            ...
            await WaitWhileMovingAsync(() => controllerY.Status.IsMoving);

            decimal target = positionInitial + Convert.ToDecimal(textBox3.Text);
            if (IsAtTarget(controllerY.DevicePosition, target, PositionTolerance))
                port.Write("B");

        }

        public async void Move_AntiClock(String name, Thorlabs.MotionControl.KCube.DCServoCLI.KCubeDCServo controllerY)
        {
